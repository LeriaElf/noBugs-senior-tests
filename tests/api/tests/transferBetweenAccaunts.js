...
describe('Transfer Service tests', function () {
  ...
  validData.forEach(({ amount, extraDeposit }) => {
    it(`User should be able to transfer valid amount - ${amount} between their accounts`, async () => {
      ...
      const { accounts: updatedAccounts } = await userSteps.getAccountWithBalance(token);
      const updatedSender = updatedAccounts.find(acc => acc.id === sender.id);
      const updatedReceiver = updatedAccounts.find(acc => acc.id === receiverAccount.id);
      ...
      const newSenderAccount = newAccauntsData.find(acc => acc.id === sender.id);
      const newReceiverAccount = newAccauntsData.find(acc => acc.id === receiverAccount.id);
  ...
  it("User should be able to transfer to someone else's account", async () => {
    ...
    const { accounts: secondAccauntsData } = await userSteps.getCustomerAccaunts(secondToken);
    ...
    const initialSenderBalance =
      sender.balance > 10000 ? AccountTransferRequest.generateTranferData() : sender.balance;
    ...
    expect(newSenderAccount.balance).to.equal(sender.balance - initialSenderBalance);

    const { accounts: newSecondAccauntsData } = await userSteps.getCustomerAccaunts(secondToken);
  ...
  it('User should not be able to transfer to a non-existent account', async () => {
    const { accountIds: newUserAccountIds, userId } = await userSteps.createUserWithAccounts(1);
    usersId.push(userId);
